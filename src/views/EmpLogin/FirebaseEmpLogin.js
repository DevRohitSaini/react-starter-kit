...
const FirebaseEmpLogin = ({ ...rest }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (userEmail === '' || password === '') {
      ...
      const loginResponse = await loginApi(constant.empLoginApi, data);

      if (loginResponse.isSuccess) {
        setSubmitted(true);
        setError(false);
        setLoggedUserDetails(loginResponse.accessToken,loginResponse.employee, 'employee');
        setUserEmail('');
        setPassword('');
          ...
          <form noValidate onSubmit={handleSubmit} {...rest}>
            ...
            <FormControl fullWidth={true} sx={{ mt: theme.spacing(3), mb: theme.spacing(1) }}>
             ...
             <Grid container justifyContent="flex-end">
              <Grid item>                
                  <RouterLink to="/employee/forget-password" size="large">Forgot Password?</RouterLink>                
              </Grid>
            </Grid>
            <Box mt={2}>
...
export default FirebaseEmpLogin;
